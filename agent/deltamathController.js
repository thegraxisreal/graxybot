...
const { DeltaMathAgent } = require('./agent-computer-use');
...
class DeltaMathController {
    ...
    async startAgent(creds = {}) {
        ...
        this.context.on('page', async (newPage) => {
            ...
            try { await newPage.setViewportSize(this.viewport); } catch(e){}
    ...
    async startComputerUseLoop() {
        if (this.isRunning) return;
        this.isRunning = true;
        console.log("🤖 Computer Use Mode Activated (GPT-5.4 mini)");
        this.io.emit('agent-thought', "Computer Use AI Activated. Analyzing screen...");

        const agent = new DeltaMathAgent(process.env.OPENAI_API_KEY, this.browser);

        try {
            const result = await agent.solveProblem(this.activePage, null, (step) => {
                if (!this.isRunning) return;
                if (step.type === 'reasoning') {
                    this.io.emit('agent-thought', step.text);
                    console.log(`💭 ${step.text}`);
                } else if (step.type === 'actions') {
                    const msg = `Step ${step.step}: ${step.actions.join(', ')}`;
                    this.io.emit('agent-thought', msg);
                    console.log(`🎯 ${msg}`);
                }
            });

            if (result) {
                this.io.emit('agent-thought', `Done: ${result}`);
                console.log(`✅ Final: ${result}`);
            }
        } catch (error) {
            console.error("Computer Use Loop Error:", error);
            this.io.emit('agent-thought', `Error: ${error.message}`);
        }
    ...
    async getAiAction(screenshotBase64) {
        try {
            const dynamicContext = `The user has already logged in and navigated to their assignment. Your goal is to solve the problems on screen.\n\n**INSTRUCTIONS:**\n1. **SOLVE:** Solve the current problem on screen — Multiple Choice or text-answer questions only.\n2. **TYPE ANSWER FIRST:** Before clicking any submit button, you MUST click the answer input and type the answer. NEVER submit a blank field.\n3. **SUBMIT:** Only after the answer is typed/selected, click submit.\n4. **NEXT PROBLEM:** After submitting, move to the next problem and repeat.`;
